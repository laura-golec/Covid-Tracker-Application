...
class Screen {
  ...
  void addWidget() {
    if (screenNumber > 0) { //Laura, adding the toolbar at the top to every screen
      textBox search;
    ...
    if (screenNumber == NATIONGRAPH) { //Laura, adding the national graphs screens
    ...
    if (screenNumber == STATESLIST) { // Laura, adding the state and count graphs screen
    ...
    // Liam adding buttons that link to the HSE website
  ...
  void draw() {
    ...
    //Aisling - map for HOME
    case HOME:
      noFill();
      background(202, 226, 245);  // Ocean colour
      rect(30, SCREENY-125, 230, 100);
      text("State: ", 35, SCREENY-100);
      text("Cases: ", 35, SCREENY-50);
      stroke(0);
      ...
      if (id != -1)
      {
        ...
        if (id ==26 || id ==11)
        {
          ...
          cases = maplist.get(10).totalCases;
        ...
        } else if (id==49 || id ==50)
        {
          ...
          fill(0);
          text(name, mouseX+5, mouseY-5);
          cases = maplist.get(48).totalCases;
          text("State: "+name, 35, SCREENY-100);
          text("Cases: "+(int)cases, 35, SCREENY-50);
        } else
        {
      ...
      break;
    case NATIONGRAPH: //Laura, drawing the graphs
      if (currentBC >= 0) barCharts[currentBC].draw();
      break;
    case SYMPTOMS: // Liam adding covid symptoms and scrollable symptoms comparison table
      fill(255);
      ...
      imageScroll.move();
      break;
    case TWEETS: // Aisling drawing Tweets
      float k=0;
      if (event ==12)
      {
        TweetsFetch();
      ...
      break;
    case STATESLIST: //Laura and Anton, drawing the graphs and the scrollbar 
      fill(255);
      rect(SCREENX - 15, 70, 10, 730);
      fill(0);
      if (currentBC >= 0) barCharts[currentBC].draw();
    ...
    if (currentScreen == screenNumber) {
      ...
      if (screenNumber != STATESLIST) { //Laura, adding all of the widgets on the screen
        for (int i = 0; i<widgetsOnScreen.size(); i++) {
          Widget aWidget = (Widget) widgetsOnScreen.get(i);
          aWidget.draw();
        }
