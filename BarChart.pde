//Laura, all of the barchart classes
class BarChart {
  int totalCasesY;
  int maxY = 1;
...
class allCasesByDateBarChart extends BarChart {
  ...
  void getArrayOfBars() { //initialises all of the data
  ...
  void draw() {
    ...
    for (int i = 0; i < dates.size(); i++) {
      ...
      if (i % datesShownMultiplier == 0) {
        pushMatrix();
        translate(cornerX + (xWidth/(dates.size()/datesShownMultiplier))*(i/datesShownMultiplier) + datesFontSize, cornerY + 10);
        rotate(angle);
...
class allStateBarChart extends BarChart {
  ...
  void getArrayOfBars() {
    ...
    for (int i = 0; i < states.size(); i++) {
      ...
      info = new ArrayList<String>();
      info.add("State: " + states.get(i).name);
      info.add("Cases: " + String.valueOf(totalCasesY));
      if (info.get(0).length() > info.get(1).length()) info.add(String.valueOf(info.get(0).length()*8));
      else info.add(String.valueOf(info.get(1).length()*8));
      if (bar.size() == i) bar.add(info);
      else bar.set(i, info);
    }
    increments = maxY;
    while (increments >= 10) {
  ...
  void draw() {
    ...
    for (int i = 0; i < stateBars.size(); i++) {
      ...
      pushMatrix();
      translate(cornerX + widthOfBars*(i+ 1), cornerY + 15);
      rotate(angle);
...
class allCountyBarChart extends BarChart {
  ...
  void getArrayOfBars() {
    ...
    for (int i = 0; i < state.counties.size(); i++) {
      ...
      if (totalCasesY > maxY) maxY = totalCasesY;
      info = new ArrayList<String>();
      info.add("County: " + state.counties.get(i).name);
      info.add("Cases: " + String.valueOf(totalCasesY));
      if (info.get(0).length() > info.get(1).length()) info.add(String.valueOf(info.get(0).length()*8));
      else info.add(String.valueOf(info.get(1).length()*8));
      if (bar.size() == i) bar.add(info);
