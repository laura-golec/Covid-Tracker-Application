...
Map minnesota, washington, montana, idaho, northDakota, maine, oregon, southDakota, newHampshire, vermont, wyoming, iowa, nebraska, massachusetts, illinois, pennsylvania, 
  conneticut, rhodeIsland, california, utah, nevada, ohio, indiana, newJersey, newYork, colorado, westVirginia, missouri, kansas, delaware, maryland, mich, virginia, kentucky, arizona, 
  oklahoma, newMexico, tennessee, northCarolina, texas, arkansas, southCarolina, alabama, m, georgia, mississippi, louisiana, florida, michigan, wisconsin;
...
void setup() {
  ...
  table = loadTable("daily-1M.csv", "header");
...
void loadData(int start, int end) { // Liam with others over shoulder
...
void keyPressed() { //Laura, this is the typing input
  if (key==CODED) {
    if (keyCode==LEFT) {
      //println ("left");
    } else {
...
//Aisling creating Tweet objects
void TweetsCreate() {
...
//Aisling calculating total cases nationwide for map
...
//Aisling: function to link cases in each state with vectors of states on map
void addStateCases ()
{
  for (int j = 0; j < maplist.size(); j++)
  {
...
//Aisling: funcitons to caluclate the states with the highest and lowest cases
void getHighestValue()
{
  for (int j = 0; j < maplist.size(); j++)
  {
    float currentCases = maplist.get(j).totalCases;
    if (currentCases > highestStateCases)
    {
